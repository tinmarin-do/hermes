...
def _regime_signal(row: dict, news: dict | None) -> dict:
    ...
    features = {
        ...
        "returns_24h": round(float(row["returns_24h"]), 6) if row["returns_24h"] is not None else None,
    }
...
def aggregate(symbols: list[str], timeframe: str = "1h") -> list[dict]:
    ...
    for symbol in symbols:
        ...
        cols = ["symbol", "timeframe", "ts", "regime", "regime_conf",
                "hurst", "garch_vol", "spread", "returns_1h", "returns_24h"]
        ...
        con.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [
            symbol, timeframe, row_dict["ts"],
            signal["regime"], signal["regime_conf"],
            row_dict["hurst"], row_dict["garch_vol"], row_dict["spread"],
            row_dict["returns_1h"], row_dict["returns_24h"],
            json.dumps(signal), computed_at,
        ])
...
def aggregate_at(symbols: list[str], timeframe: str, as_of_ts: datetime) -> list[dict]:
    ...
    for symbol in symbols:
        row = con.execute("""
            ...
            WHERE a.rn = 1
        """, [symbol, timeframe, as_of_ts, symbol, timeframe, as_of_ts]).fetchone()
...
def get_forward_return(symbol: str, timeframe: str, from_ts: datetime,
    ...
    try:
        row = con.execute("""
            ...
            LIMIT 1
            OFFSET ?
        """, [symbol, timeframe, from_ts, periods - 1]).fetchone()
        if row is None:
            return None
        ...
        start = con.execute("""
            ...
            WHERE symbol = ? AND timeframe = ? AND ts = ?
        """, [symbol, timeframe, from_ts]).fetchone()
        if start is None:
            return None
